...
class Model_Var_Cost(nn.Module):
    ...
    def forward(self, dx, dy):
        loss = self.alphaReg**2 * self.normPrior(dx,self.epsReg)
                
        if self.DimObs == 1 :
            loss +=  self.alphaObs[0]**2 * self.normObs(dy,self.epsObs[0])
        else:
            for kk in range(0,self.DimObs):
                loss +=  self.alphaObs[kk]**2 * self.normObs(dy[kk],self.epsObs[kk])
