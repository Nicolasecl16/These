...
def save_NetCDF(saved_path1, mu , sigma, init, target ):    
        N = mu.shape[0]
        print(N)
        dT = mu.shape[1]
        dX = mu.shape[2]
        ...
        print(Nb)
        xrdata = xr.Dataset( 
                        data_vars=dict(mean_pred =(["Nb_samples", "time","x"],mu),
                                       cov_pred =(["Nb_samples", "time","x"],sigma),
                                       inits =(["Nb_samples", "time","x"],init[:,:,:,0]),
