...
class Model_4DVarNN_GradFP(torch.nn.Module):
    def __init__(self,phi_r,ShapeData,NiterProjection,NiterGrad,GradType,OptimType,InterpFlag=False,periodicBnd=False):
    ...
    def forward(self, x_inp,xobs,mask,g1=None,g2=None,normgrad=0.0):
        ...
        if self.NGrad > 0:
            ...
            grad     = self.model_Grad.compute_Grad(x, self.phi_r(x),xobs,mask)
            if normgrad == 0. :
